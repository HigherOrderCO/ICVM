...
pub fn term_to_bitstring(t: &Term) -> Vec<u8> {
  fn format_binary_output(t: &Term, v: &mut Vec<u8>) {
    match t {
      Term::Lam { nam: ref o_nam, typ: _, bod: ref o_bod } => {
        // TODO: handle typ
        match **o_bod {
          Term::Lam { nam: ref i_nam, typ: _, bod: ref i_bod } => {
            // TODO: handle typ
            match **i_bod {
              Term::Lam { nam: _, typ: _, bod: ref e_bod } => {
                // TODO: handle typ
                match **e_bod {
                  Term::App { fun: ref app_fun, arg: ref app_arg } => match **app_fun {
